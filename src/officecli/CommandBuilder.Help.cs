    ...
    private static readonly Dictionary<string, string[]> EarlyDispatchHelp =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["issue"] = new[]
            {
                ...
                "Additive-only surface: closing/status changes go through omc (herdr-delegated).",
                "Agents: hit a defect mid-session — open it one-key with an acceptance line.",
            },
            ["artifact"] = new[]
            {
                "Usage:",
                "  officecli artifact publish --name <n> --kind experience|lesson|research --digest sha256:<64hex>",
                "      --summary \"<experience description>\" --outcome success|failure",
                "      [--version <v>] [--git-range <a..b>] [--deps <id,...>] [--note <t>]",
                "  officecli artifact attest <id> --type attest_dev|attest_prod|verification_failed",
                "  officecli artifact list [--current] [--env dev|prod] [--json]",
                "",
                "Three-type standard (总台 2026-09-22): experience, lesson, research;",
                "research carries the git landing path via --git-range. Summary (text",
                "body) + outcome (success|failure) required — empty publishes are",
                "rejected locally first, 400 server-side. Digest = sha256 of content/record.",
                "Promote/demote/supersede are NOT in this CLI (additive-only surface): they run",
                "through the omc workbench (herdr-delegated).",
            },
            ["config"] = new[]
            {
                "Usage:",
